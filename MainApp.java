...
public class MainApp {
  ...
  private final JLabel
      totalMealsLabel; // Etykieta do wyświetlania liczby obiadów
  ...
  private static final String MISSING_CARD_COUNT_FILE =
      "data/missingCardCount.txt";
  ...
  public MainApp() {
    String[] columnNames = {"Imię", "Nazwisko", "Klasa", "Nr w dzienniku",
                            "ID Karty"};
    ...
    endButton.addActionListener(e -> {
      int result = JOptionPane.showConfirmDialog(
          mainPanel, "Czy na pewno chcesz zakończyć aplikację?",
    ...
    inputField.getInputMap(JComponent.WHEN_FOCUSED)
        .put(KeyStroke.getKeyStroke("ENTER"), "addStudent");
    ...
    try (BufferedReader reader =
             new BufferedReader(new FileReader(MEALS_FILE_PATH))) {
    ...
    if (!shutdownHookRegistered) {
      Runtime.getRuntime().addShutdownHook(
          new Thread(() -> { System.out.println("Program zamknięto"); }));
...
private void handleInputChange() {
  ...
  private void logToRaportFile(String message) {
    try (BufferedWriter writer =
             new BufferedWriter(new FileWriter(REP_FILE_PATH, true))) {
  ...
  /**
   * Aktualizuje plik, w którym zapisywane są informacje, kto ile razy zapomniał
   * kartę.
   */
  ...
  private void loadStudentData() {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(CSV_FILE))) {
      ...
      JOptionPane.showMessageDialog(
          null, "Błąd podczas wczytywania pliku CSV: " + e.getMessage());
    }
  }
  ...
  private void generateMealSummary() {
    ...
    JOptionPane.showMessageDialog(
        mainPanel, "Podsumowanie obiadów zapisane do pliku: " + SUM_FILE_PATH);
  }

  private void writeSummaryToFile() {
    LocalDateTime now = LocalDateTime.now();
    String dateHeader = "===== dzień: " + now.getDayOfMonth() +
                        " === miesiąc: " + now.getMonthValue() +
                        " === rok: " + now.getYear() + " =====";
    try (BufferedWriter writer =
             new BufferedWriter(new FileWriter(Data_Sum_FILE_PATH, true))) {
  ...
  private void processInput(String input) {
    ...
    if (input.matches("\\d{10}") ||
        (input.length() > 8 && input.matches("\\d+"))) {
  ...
  private void addStudentById(String cardId) {
    for (Student student : studentList) {
      if (student.getCardId().equalsIgnoreCase(cardId)) {
        ...
        totalStudents++;
        logToFile("Dodano ucznia: " + student.getFirstName() + " " +
                  student.getLastName() + " (ID: " + cardId + ")");
        return;
      }
    }
    // Jeśli ucznia nie znaleziono – zakładamy, że zapomniał karty
    String message = LocalDateTime.now() + " - Uczeń o identyfikatorze " +
                     cardId + " zapomniał karty";
    logToRaportFile(message);
    JOptionPane.showMessageDialog(null,
                                  "Nie znaleziono ucznia z ID: " + cardId +
                                      ". Być może zapomniał karty.");
  }
   ...
   */
  private void addStudentByClassAndNumber(String className,
                                          String classNumber) {
    for (Student student : studentList) {
      if (student.getClassName().equalsIgnoreCase(className) &&
        ...
        addStudentToTable(student);
        logToFile("Dodano ucznia: " + student.getFirstName() + " " +
                  student.getLastName() + " (klasa: " + student.getClassName() +
                  ", nr: " + student.getClassNumber() + ")");
        // Logowanie informacji, że uczeń zapomniał kartę
        String message = LocalDateTime.now() + " - Uczeń klasy " +
                         student.getClassName() + student.getClassNumber() +
                         " zapomniał kartę (" + student.getFirstName() + " " +
                         student.getLastName() + ")";
        logToRaportFile(message);
    ...
    String errorMessage =
        "Nie znaleziono ucznia w klasie: " + className + ", nr: " + classNumber;
    JOptionPane.showMessageDialog(null, errorMessage);
    logToFile("Błąd: " + errorMessage);
  }
  ...
  private void addTeachers(int count) {
    for (int i = 1; i <= count; i++) {
      ...
      tableModel.addRow(
          new Object[] {teacherName, "Nowak", "Brak", "Brak", "Brak"});
    }
    totalMeals += count;
    totalTeachers += count;
    configureRowColoring();
  ...
  private void addStudentToTable(Student student) {
    tableModel.addRow(new Object[] {
        ...
        student.getClassNumber(), student.getCardId()});
    try (BufferedWriter writer =
             new BufferedWriter(new FileWriter(LOG_FILE_PATH, true))) {
      writer.write("Dodano studenta: " + student.getFirstName() +
    ...
    dataTable.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
      ...
      public Component getTableCellRendererComponent(
        ...
        for (int i = 0; i < table.getRowCount(); i++) {
          ...
          if (!uniqueIds.add(id) && id.equals(cardId) && id != null &&
              !id.isEmpty()) {
            isDuplicate = true;
            break;
          }
  ...
  private void updateMealsLog() {
    ...
    String message = mealsLogCount + ". " + LocalDateTime.now() +
                     ...
                     " (Dzieci: " + totalChildren +
                     ", Nauczyciele: " + totalTeachers +
                     ", Uczniowie: " + totalStudents + ")";
  ...
  private void appendTodayDateToFile(String filePath) {
    try {
      LocalDateTime now = LocalDateTime.now();
      String todayHeader = "===== dzień: " + now.getDayOfMonth() +
                           " ==== miesiąc: " + now.getMonthValue() + " =====";
  ...
  private void saveTotalMealsSummary() {
    try (BufferedWriter writer =
             new BufferedWriter(new FileWriter(SUM_FILE_PATH))) {
  ...
  private void saveStudentData() {
    try (BufferedWriter bw = new BufferedWriter(new FileWriter(CSV_FILE))) {
      ...
      JOptionPane.showMessageDialog(
          null, "Błąd podczas zapisywania pliku CSV: " + e.getMessage());
  ...
  public static void main(String[] args) {
    ...
    SwingUtilities.invokeLater(() -> {
      ...
      // Ustawiamy tryb zamykania okna na DO_NOTHING_ON_CLOSE i komunikat o
      // konieczności kliknięcia przycisku "Koniec"
      frame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
      frame.addWindowListener(new WindowAdapter() {
        @Override
        public void windowClosing(WindowEvent e) {
          JOptionPane.showMessageDialog(
              frame, "Aby zamknąć aplikację, kliknij przycisk 'Koniec'");
        }
      });
      frame.pack();
